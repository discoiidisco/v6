...
if (!isset($_SESSION['setup']['permissions'])) {
    ...
    if (!isset($_SESSION['setup']['global']) || !isset($_SESSION['setup']['progress'])) {
        ...
        if (file_put_contents($global_file, $config)) {
            ...
            $default_docs = array(
                0 => array('doc_name' => str_replace('CubeCart', $_SESSION['setup']['config']['store_name'], $strings['setup']['default_doc_title_welcome']), 'doc_content' => $strings['setup']['default_doc_content_welcome'], 'doc_order' => 1, 'doc_lang' => $config['default_language'], 'doc_home' => 1, 'doc_terms' => 0, 'doc_privacy' => 0, 'hide_title' => 1),
